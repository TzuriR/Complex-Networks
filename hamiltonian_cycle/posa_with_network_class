...
def make_cycle(net, rail_v, rail_e):
    ...
    if (is_at(net.edges, (rail_v[0], rail_v[len(rail_v) - 1])) == 1) or (
        ...
        edge_temp = net.find_edge_between_vertices(rail_v[len(rail_v) - 1], rail_v[0])
...
def swap_cycle(net, x0, xi, xi_plus_one, xn, rail_v, rail_e):
    ...
    while rail_v[index] != xi_plus_one:
        ...
        index += 1
    temp_v.append(xn)
    edge_temp_1 = net.find_edge_between_vertices(xi, xn)
    rail_e.append(edge_temp_1)
    # temp_e.append((xi, xn))
    ...
    while end_index >= rail_v.index(xi_plus_one):
        ...
        edge_temp_2 = net.find_edge_between_vertices(rail_v[end_index + 1], rail_v[end_index])
        rail_e.append(edge_temp_2)
        # temp_e.append((rail_v[end_index + 1], rail_v[end_index]))  # How direct from rail_e?
        end_index -= 1
        index += 1
    edge_temp_3 = net.find_edge_between_vertices(xi_plus_one, x0)
    rail_e.append(edge_temp_3)
