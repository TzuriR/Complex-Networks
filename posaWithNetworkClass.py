...
def posa_loop(net, x, rail_v, rail_e):
    ...
    if len(rail_v) == len(net.nodes):
        ...
        rail_v, rail_e = make_cycle(net, rail_v, rail_e)
        print("rail_v:")
        net.print_point_arr(rail_v)
        print("rail_e:")
        net.print_point_arr(rail_e)
    # Check if the path is not hamiltonian - go back
        ...
        rail_v, rail_e = posa_loop(net, rail_v[len(rail_v) - 1], rail_v, rail_e)
    return rail_v, rail_e


# --------------------------------------------------------------------------------
# Utility function- get hamiltonian path and convert it to hamiltonian cycle
def make_cycle(net, rail_v, rail_e):
    # If there is (x0, xn) - add it to the rail and finish
    if (is_at(net.edges, (rail_v[0], rail_v[len(rail_v) - 1])) == 1) or (
            is_at(net.edges, (rail_v[len(rail_v) - 1], rail_v[0])) == 1):
        edgeTemp = net.find_edge_between_vertices(rail_v[len(rail_v) - 1], rail_v[0])
        rail_e.append(edgeTemp)
    ...
    for v in rail_v:
        ...
        if ((is_at(net.edges, (x0, xi_plus_one)) == 1) or (is_at(net.edges, (xi_plus_one, x0)) == 1)) and (
                (is_at(rail_v, (xi_plus_one, x0)) == 0) or (is_at(rail_v, (x0, xi_plus_one)) == 0)):
            if ((is_at(net.edges, (xi, xn)) == 1) or (is_at(net.edges, (xn, xi)) == 1)) and (
                    (is_at(rail_v, (xn, xi)) == 0) or (is_at(rail_v, (xi, xn)) == 0)):
                if is_at(rail_e, (xi, xi_plus_one)) == 1:
                    rail_v, rail_e = swap_cycle(net, x0, xi, xi_plus_one, xn, rail_v, rail_e)
...
def swap_cycle(net, x0, xi, xi_plus_one, xn, rail_v, rail_e):
    temp_v = []
    temp_e = []
    index = 0
    while rail_v[index] != xi_plus_one:
        ...
        if index != 0:
            edgeTemp = net.find_edge_between_vertices(rail_v[index - 1], rail_v[index])
            rail_e.append(edgeTemp)
            #temp_e.append((rail_v[index - 1], rail_v[index]))
        index += 1
    temp_v.append(xn)
    edgeTemp1 = net.find_edge_between_vertices(xi, xn)
    rail_e.append(edgeTemp1)
    #temp_e.append((xi, xn))
    ...
    while end_index >= rail_v.index(xi_plus_one):
        ...
        edgeTemp2 = net.find_edge_between_vertices(rail_v[end_index + 1], rail_v[end_index])
        rail_e.append(edgeTemp2)
        #temp_e.append((rail_v[end_index + 1], rail_v[end_index]))  # How direct from rail_e?
        end_index -= 1
        index += 1
    edgeTemp3 = net.find_edge_between_vertices(xi_plus_one, x0)
    rail_e.append(edgeTemp3)
...
def rot_ext(net, rail_v, rail_e):
    ...
    for xi in adj_x:
        ...
        for xk in adj_xi_plus_one:
            ...
            rail_v, rail_e = swap_rail(net, xi, xi_plus_one, xk, xt, rail_v, rail_e)
            flag = 1  # we back from swap
            break
        break
    if flag == 0:
...
def swap_rail(net, xi, xi_plus_one, xk, xt, rail_v, rail_e):
    temp_v = []
    temp_e = []
    index = 0
    while rail_v[index] != xi_plus_one:
        ...
        if index != 0:
            edgeTemp = net.find_edge_between_vertices(rail_v[index - 1], rail_v[index])
            temp_e.append(edgeTemp)
            #temp_e.append((rail_v[index - 1], rail_v[index]))
        index += 1
    temp_v.append(xt)
    edgeTemp = net.find_edge_between_vertices(xi, xt)
    temp_e.append(edgeTemp)
    #temp_e.append((xi, xt))
    # endIndex run on rail_v, index run on tempV
    index += 1
    ...
    while end_index >= rail_v.index(xi_plus_one):
        temp_v.append(rail_v[end_index])
        edgeTemp = net.find_edge_between_vertices(rail_v[end_index + 1], rail_v[end_index])
        temp_e.append(edgeTemp)
        #temp_e.append((rail_v[end_index + 1], rail_v[end_index]))  # How direct from rail_e?
        end_index -= 1
        index += 1
    temp_v.append(xk)
    edgeTemp = net.find_edge_between_vertices(xi_plus_one, xk)
    temp_e.append(edgeTemp)
...
def insert_to_rail(net, x, rail_v, rail_e):
    ...
    for y in adj_x:
        if is_at(rail_v, y) == 0:
            rail_v.append(y)
            edgeTemp = net.find_edge_between_vertices(x, y)
            rail_e.append(edgeTemp)
